...
/** Walks from the start at (3,2) to (3,4) — the Dirt holding up the boulder stack — and stops
 * there, directly underneath it. Two accepted moves. */
const UNDERMINE_AND_STAND_UNDER = ["ArrowRight", "ArrowRight"];
...
test("walking into a boulder is a rejected move, never a death", async ({ page }) => {
  ...
  // (2,4) is a boulder — the move must be refused, and refusing it must not cost a move.
  await pressKeys(page, ["ArrowRight"]);

  await expectPlayerAt(page, 2, 3);
  await expectInputResponseText(page, "2:2,3");
  await expectLevelStatus(page, "active");
  await expectLossCause(page, "none");
});
...
test("a supported boulder overhead is safe indefinitely", async ({ page }) => {
  ...
  // Stand beside the stack without touching the Dirt at (3,4) that holds it up.
  await pressKeys(page, ["ArrowRight"]);
  await expectPlayerAt(page, 3, 3);
