...
169 tests across 11 spec files: `guardrails`, `digging`, `boulder-gravity`, `boulder-roll`, `boulder-crush`, `undermine-gated-gem`, `game-clock`, `level-progression`, `high-score`, `level-invariants`, `level-solver`. (`guardrail-assertions.ts` is a shared helper, not a spec.)
